...
struct
  ...
  *)
  ...
  let mem x s =
    let h = M.hash x in
    try
      let m = Int.Map.find (Int32.to_int h) s in
      Set.mem x m
  ...
  let add x s =
    ...
    try
      ...
      Int.Map.set (Int32.to_int h) m s
    with Not_found ->
      let m = Set.singleton x in
      Int.Map.add (Int32.to_int h) m s

  let singleton x =
    let h = M.hash x in
    let m = Set.singleton x in
    Int.Map.singleton (Int32.to_int h) m
  ...
  let remove x s =
    ...
    try
      ...
      if Set.is_empty m then
        Int.Map.remove (Int32.to_int h) s
      else
        Int.Map.set (Int32.to_int h) m s
    with Not_found -> s
...
struct
  ...
  *)
  ...
  let singleton k x =
    let h = M.hash k in
    Int.Map.singleton (Int32.to_int h) (Map.singleton k x)
  ...
  let find_opt k s =
    let h = M.hash k in
    match Int.Map.find_opt (Int32.to_int h) s with
    | None -> None
  ...
  let update k f m =
    ...
    in
    Int.Map.update (Int32.to_int @@ M.hash k) aux m
