...
let hashcons (sh_sort,sh_ci,sh_construct,sh_ind,sh_con,sh_na,sh_id) =
  let rec hash_term t =
	...
	let t, ht = sh_rec t in
        (Cast (c, k, t), combinesmalli 3 (combine3 hc (Hashval.of_int (hash_cast_kind k)) ht))
      | Prod (na,t,c) ->
	let t, ht = sh_rec t
	and c, hc = sh_rec c in
        (Prod (sh_na na, t, c), combinesmalli 4 (combine3 (hash_annot Name.hash na) ht hc))
      | Lambda (na,t,c) ->
	let t, ht = sh_rec t
	and c, hc = sh_rec c in
        (Lambda (sh_na na, t, c), combinesmalli 5 (combine3 (hash_annot Name.hash na) ht hc))
      | LetIn (na,b,t,c) ->
	let b, hb = sh_rec b in
	let t, ht = sh_rec t in
	let c, hc = sh_rec c in
        (LetIn (sh_na na, b, t, c), combinesmalli 6 (combine4 (hash_annot Name.hash na) hb ht hc))
      | App (c,l) ->
	let c, hc = sh_rec c in
	...
	let u', hu = sh_instance u in
        (Const (c', u'), combinesmalli 9 (combine (Constant.SyntacticOrd.hash c) (Hashval.of_int hu)))
      | Ind (ind,u) ->
	let u', hu = sh_instance u in
	(Ind (sh_ind ind, u'), 
         combinesmalli 10 (combine (ind_syntactic_hash ind) (Hashval.of_int hu)))
      | Construct (c,u) ->
	let u', hu = sh_instance u in
	(Construct (sh_construct c, u'),
         combinesmalli 11 (combine (constructor_syntactic_hash c) (Hashval.of_int hu)))
      | Case (ci,p,c,bl) ->
	let p, hp = sh_rec p
	and c, hc = sh_rec c in
...
let rec hash t =
  match kind t with
    ...
    | Cast (c, k, t) ->
      let hc = hash c in
      let ht = hash t in
      combinesmalli 3 (combine3 hc (Hashval.of_int (hash_cast_kind k)) ht)
