...
struct
  ...
  let hash x = Int32.of_int @@ String.hash x
...
struct
  ...
  let hash dp = hash Int32.zero dp
...
struct
  ...
  let hash (i, id, dp) =
    combine3 (Int32.of_int i) (Id.hash id) (DirPath.hash dp)
...
module ModPath = struct
  ...
  | MPfile dp -> combinesmalli 1 (DirPath.hash dp)
  | MPbound id -> combinesmalli 2 (MBId.hash id)
  | MPdot (mp, lbl) ->
    combinesmalli 3 (combine (hash mp) (Label.hash lbl))
...
module KerName = struct
  ...
  let make modpath dirpath knlabel =
    { modpath; dirpath; knlabel; refhash = Int32.minus_one; }
  let repr kn = (kn.modpath, kn.dirpath, kn.knlabel)

  let make2 modpath knlabel =
    { modpath; dirpath = DirPath.empty; knlabel; refhash = Int32.minus_one; }
  ...
  let equal kn1 kn2 =
    ...
    if Int32.compare Int32.zero h1 <= 0 && Int32.compare Int32.zero h2 <= 0 && not (Int32.equal h1 h2) then false
    else
  ...
  let hash kn =
    let h = kn.refhash in
    if Int32.compare Int32.zero h > 0 then
...
let ind_hash (m, i) =
  Hashset.Combine.combine_i (MutInd.hash m) (Int.hash i)
let ind_user_hash (m, i) =
  Hashset.Combine.combine_i (MutInd.UserOrd.hash m) (Int.hash i)
let ind_syntactic_hash (m, i) =
  Hashset.Combine.combine_i (MutInd.SyntacticOrd.hash m) (Int.hash i)
...
let constructor_hash (ind, i) =
  Hashset.Combine.combine_i (ind_hash ind) (Int.hash i)
let constructor_user_hash (ind, i) =
  Hashset.Combine.combine_i (ind_user_hash ind) (Int.hash i)
let constructor_syntactic_hash (ind, i) =
  Hashset.Combine.combine_i (ind_syntactic_hash ind) (Int.hash i)
...
struct
  module Repr = struct
    ...
    let hash p =
      Hashset.Combine.combinesmalli p.proj_arg (ind_hash p.proj_ind)
    ...
    module UserOrd = struct
      ...
      let hash p =
        Hashset.Combine.combinesmalli p.proj_arg (ind_user_hash p.proj_ind)
    end
  ...
  let hash (c, b) = Hashset.Combine.combinesmalli (if b then 0 else 1) (Repr.hash c)
  ...
  module SyntacticOrd = struct
    ...
    let hash (c, b) = Hashset.Combine.combinesmalli (if b then 0 else 1) (Repr.SyntacticOrd.hash c)
  end
  module CanOrd = struct
    let compare (c, b) (c', b') =
    ...
    let hash (c, b) = Hashset.Combine.combinesmalli (if b then 0 else 1) (Repr.CanOrd.hash c)
  end
